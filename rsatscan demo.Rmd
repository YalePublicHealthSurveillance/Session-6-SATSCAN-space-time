...
```{r}
shape.NYC <- sf::read_sf(dsn = "./Data/NYC shape files", layer = "geo_export_ef7f38de-fdc9-46c6-bf6b-36dcb40dfda8")
```
...
```{r}
shape.results1 <- sf::st_read(dsn = "./Manual results/NYC_resp.gis.shp")
shape.results2 <- sf::st_read(dsn = "./Manual results/NYC_resp.col.shp")
shape.results.points <- sf::st_read(dsn = "./Manual results/NYC_resp.gis.shp")

shapefile_list <- list(shape.NYC,shape.results1,shape.results2,shape.results.points)

# Base plot with the first shapefile
p <- ggplot() +
  geom_sf(data = shapefile_list[[1]], fill = "white", color = "black", alpha = 0.6)

# Overlay remaining shapefiles with different colors
for (i in 2:length(shapefile_list)) {
  p <- p + geom_sf(data = shapefile_list[[i]], fill = NA, color = sample(colors(), 1), size = 0.8)
}
